...
def Sideways_Move__HILL_CLIMBING(problem):
    sideways_move = 0
    current = problem.initial
    print( "initial, ", - problem.value(current),"state=",current)
    while True:
        ...
        if c1 <= problem.value(current):
            sideways_move += 1
            if sideways_move >= 10 or problem.isGoal(current):
...
Sideways_Move__HILL_CLIMBING(probleme)